    ...
    def on_double_click(self, event):
        ...
        def update_values(tag):
            ...
            if tag == "book":
                ...
                Employee.update_employee(dialog, self.db_connection,"Admin", new_values, values)
        ...
        def delete_values(tag):
            if tag == "book":
                ...
                Employee.delete_employee(dialog, self.db_connection,"Admin", values)
                Employee.configure_employee_treeview(self.db_connection, self.tree) 
            elif tag == "user":
        ...
        if employee_list_menu == "Gestione Dipendenti":
            ...
            def get_datas():
                ...
                if password_var.get() == "" or len(password_var.get()) <= 8:
                    ...
                    send = False
                if send == True:
                    Employee.add_employee(dialog, self.db_connection, "Admin", nome_var.get(), cognome_var.get(), email_var.get(), user_var.get(), password_var.get(), ruolo_entry.get())
                    Employee.configure_employee_treeview(self.db_connection, self.tree)
            # Crea il pulsante di conferma
